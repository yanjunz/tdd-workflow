...
支持 **Claude Code** · **Cursor** · **Cline** · **Windsurf** · **GitHub Copilot**。
...
| 规范模板 | `requirements.md`、`design.md`、`tasks.md` | `.{tool}/skills/tdd-workflow/templates/` |
...
1. **RED** — 写一个失败的测试（确认是因为功能未实现而非语法错误）
2. **GREEN** — 写最少代码让测试通过（每次都跑全量回归）
3. **REFACTOR** — 消除重复、改善命名

```
[RED]      comment-service.test.ts: createComment — FAIL ✗ (expected)
[GREEN]    comment-service.ts: createComment — 5/5 tests PASS ✓
...
| **支持工具数量** | 17+（含 Codex、Amazon Q、Augment 等） | 5 个 |
