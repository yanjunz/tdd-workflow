...
**选 [D] 时的交叉验证**：
- 用最基础的方式验证服务可用性（如 `curl` 直接访问端口、`lsof -i:<port>` 检查进程）
- 如果基础方式确认在线但 readiness 脚本报失败 → readiness 脚本有 bug → 要求修复脚本后再写入配置
- 不能信任有 bug 的脚本写入 project.md
...
- **readiness 脚本也需要验证正确性** — 不是"能跑"就够了，还要"已知在线时返回 pass"
