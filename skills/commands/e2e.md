...
- **跨 UC 数据流标注**：如果某 UC 的展示数据来源是另一个 UC（如"心跳上报 → 页面展示"），在 E2E 清单中标注 `[跨UC: UC-XX→UC-YY]`，编写时必须断言实际值（Rule 7）
...
#### 7.1 证据留存（有 UI 的端 mandatory）

对于 `type=browser` 和 `type=device` 端的 E2E，必须保留**可视化证据**，让用户可审计验证过程：

**截图要求**：
- `type=browser`：配置 `screenshot: 'on'` 或在关键断言点 `page.screenshot()`
- `type=device`：在关键断言点调用框架的截图能力（如 minium `mini.screen_shot()`）
- 截图保存到 `tdd-specs/<feature>/evidence/`（随 feature 归档，可追溯）

**运行完后主动展示**：
- 测试通过后，用 Read tool 读取关键截图展示给用户
- 每端至少展示 1-2 张核心断言点的截图
- 输出格式：

```
E2E 证据：
  [截图 1] UC-05 成功路径 — 打印机列表含"客户端版本"列 (web-admin)
  [截图 2] UC-02 成功路径 — 订单详情展示取件码 (miniprogram)
```

**不能只靠文字声称"通过了"** — 用户需要看到 UI 截图才能确认 AI 真的验证过。

对于 `type=api` 端：截图不适用，但应保留关键响应数据摘要（如 API 返回的 JSON 片段）作为证据。
...
### Rule 7: Cross-UC data flow must assert actual values

当展示类 UC 的数据来源是**另一个 UC**（如"UC-01 心跳上报版本 → UC-05 页面展示版本"），E2E 不能只断言"有数据渲染"，**必须断言"显示的值 = 上游实际值"**。

```javascript
// WRONG: 只断言有值存在
const versionCell = page.locator('td').filter({ hasText: /v\d+/ });
expect(await versionCell.count()).toBeGreaterThan(0); // ← 不管值对不对

// CORRECT: 断言显示的值等于上游源数据
// 方式 1: 从已知数据源获取期望值
const expectedVersion = 'v1.1.18'; // 从 VERSION 文件 / API 响应获取
await expect(page.locator('td').filter({ hasText: expectedVersion })).toBeVisible();

// 方式 2: 先通过 API 写入，再验证页面展示一致
const apiResp = await request.get('/api/printers');
const reportedVersion = apiResp.data[0].clientVersion;
await expect(page.locator('td').filter({ hasText: reportedVersion })).toBeVisible();
```

**识别跨 UC 数据流的信号**：
- UC 的"相关数据"字段标注了"由 XX 上报""来自 XX 接口"
- UC 步骤里出现"展示 XX 上报的 YY"
- 数据不是本 UC 产生的，而是依赖另一端/另一个 UC 的写入

**遇到跨 UC 数据流时**：在 Step 4 派生 E2E 清单时标注 `[跨UC]`，提醒编写时必须验证值的正确性而非仅存在性。
...
- **有 UI 的端 E2E 必须留截图证据** — browser + device 端跑完后主动向用户展示关键截图（Read tool 读图片），不能只靠文字 pass/fail 声称验证过
- **跨 UC 数据流必须断言实际值** — 展示类 UC 如果数据来源是另一个 UC/端，不能只断言"有值"，必须断言"值 = 上游实际值"（Rule 7）。看到截图里有数据不等于数据正确
