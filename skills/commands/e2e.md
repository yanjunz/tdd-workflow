...
```bash
# 检查 project.md 中 environments.dev.readiness 定义的健康检查
<environments.dev.readiness> 2>/dev/null && echo "OK Service online" || echo "WARNING: Service not running, please start dev server first"
```
